  ...
  >
    <v-row class="d-flex ml-8" no-gutters>
      ...
      <v-column class="d-flex align-center justify-space-around mr-8">
        <v-icon
          class="mr-4"
          @click="window.location.href = 'twitter.com'"
          color="blue"
          >mdi-twitter</v-icon
        >
      ...
      <v-column class="d-flex align-center mr-8">
        <v-dialog v-model="dialog" max-width="400">
          ...
          <v-card>
            <v-card-title>
              <span class="text-h5"
                >Donate to <b>{{ creator.name }}</b></span
              >
            </v-card-title>
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" class="pt-0">
                    <v-textarea label="Your Message" required></v-textarea>
                  </v-col>
                </v-row>
              </v-container>
...
export default {
  async fetch() {
    this.creator = await this.$store.dispatch("api/getUser", this.$auth.id);
